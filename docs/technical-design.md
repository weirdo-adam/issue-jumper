...
v0.1.0 的设计目标是提供编辑器无关的 CLI 能力，用于从当前 Git 分支推导目标 Issue URL 并打开系统默认浏览器。Zed 集成通过 task 和 keymap 调用同一 CLI，使用户可以在当前 worktree 上触发跳转。成功路径不得依赖 Agent、人工复制 Issue ID 或中间链接点击。

Issue Jumper 采用“约定优先、配置覆盖”的策略：GitHub、GitLab、私有 GitLab 以及常见分支命名规则默认可用；Redmine、Jira、GitLab work items 和企业内部工单系统等非统一 URL 形态通过可选配置覆盖。
...
Zed 集成由 `tasks.json` 和 `keymap.json` 完成。CLI 负责 Git 信息读取、Issue ID 匹配、URL 构建以及系统浏览器调用。当前公开 Zed extension API 的 slash command 入口属于 Agent UI，不属于 Command Palette action，因此 v0.1.0 不采用 `/issue` 路径。后续其他编辑器或启动器集成应作为新的 `install-*` 适配层加入，并复用核心解析逻辑。
...
| 快捷键触发 | Zed keymap 绑定 `task::Spawn` |
...
| 配置扩展 | 读取全局配置，并支持 `.zed/issue-jumper.json` 或 `.issue-jumper.json` 项目级覆盖 |
| 失败反馈 | CLI 输出结构化错误，Zed task terminal 保留诊断信息 |
| 隐私与离线 | 核心解析在本机完成，不采集客户数据，不包含遥测；安装后 `url`、`doctor` 等命令可离线使用 |

### 1.4 验收标准

| 验收项 | 必须满足 |
| --- | --- |
| 快捷键可用 | 在 Zed Workspace 或文件编辑器焦点中按配置快捷键即可触发跳转 |
| 无中间交互 | 成功路径不得要求确认、复制、点击或手动输入命令 |
| 浏览器打开 | 系统默认浏览器打开 Issue URL |
| 当前仓库正确 | 使用当前 Zed worktree 对应的 Git 仓库 |
| 当前分支正确 | 从当前 Git 分支提取 Issue ID |
| 跳转目标正确 | URL 对应当前仓库 remote 和提取出的 Issue ID |
| 失败可诊断 | 失败输出包含明确原因，如无匹配规则、非 Git 仓库、无 remote |

### 1.5 非目标

- v0.1.0 不发布 Zed marketplace extension。
- v0.1.0 不依赖未公开的 Zed Action API，也不宣称能注册独立 `issue: open` Command Palette action。
- 快捷键路径不得依赖 Agent、Assistant 或 Markdown 链接点击。
- v0.1.0 不调用 Issue 平台 API，无需用户 token。
- v0.1.0 不采集客户数据，不上传分支名、仓库路径、Git remote、配置值或 Issue ID。
- v0.1.0 不依赖在线服务完成本地解析；安装脚本下载 Release 资产和浏览器访问远端 Issue 页面属于外部网络行为。
- v0.1.0 不自动创建 Issue，只打开已有 Issue URL。

### 1.6 命名与标识

项目名和用户可见 task 名统一使用 `Issue Jumper`；仓库名、命令名和 Cargo package 使用不绑定具体编辑器的 `issue-jumper`。Zed 相关名称仅用于安装子命令、task label 和配置路径。

| 对象 | 名称 | 说明 |
| --- | --- | --- |
| 产品展示名 | `Issue Jumper` | 用户可见主名称，表达从分支跳转到 Issue 页面的能力 |
| 项目/仓库名 | `issue-jumper` | 不绑定单一编辑器，支持后续扩展其他入口 |
...
| Zed task label | `Issue Jumper: Open Current Issue` | 描述快捷键触发的动作 |
| 配置文件 | 全局 `issue-jumper/config.json`，项目 `.issue-jumper.json` / `.zed/issue-jumper.json` | 核心 CLI 配置和项目覆盖路径保持稳定 |
| README 副标题 | `Jump from the current Git branch to its issue page` | 概括核心 CLI 能力 |
...
- keymap 可以通过 `task::Spawn` 绑定指定 task。
- Command Palette 中可执行 Zed 内置 `task: spawn`，再选择已安装的 `Issue Jumper: Open Current Issue` task。

当前公开 Zed extension API 未提供将自定义 action 直接注册到 Command Palette 的 manifest/API 字段。`slash_commands` 属于 Agent UI 入口，不满足本项目的交互目标。

### 2.2 CLI 执行环境

CLI 可以在终端、脚本或 Zed task 进程中运行。`install-zed` 写入当前 CLI 的绝对路径，以规避 Zed task 环境与交互式 shell 的 PATH 差异。task 通过 `args` 传递 `$ZED_WORKTREE_ROOT`，CLI 使用该路径读取 Git 信息。
...
失败时不实现自定义 Zed UI。CLI 返回非零 exit code，并由 task terminal 保留错误输出：

```text
Issue Jumper failed: no issue ID matched branch "main".
```

GitHub 或 GitLab remote 下未识别到 Issue ID 时执行仓库级跳转，CLI 打开远程仓库首页；其他平台仍返回无匹配规则错误。

task 配置默认 `reveal = "never"`、`hide = "on_success"`，成功时不显示任务终端；失败时终端保留输出，用于定位问题。

## 四、Zed 集成设计

### 4.1 Command Palette 边界

Zed Command Palette 的 `Execute a command...` 入口执行内置 action。v0.1.0 不注册独立的 `issue: open` action，因为当前公开 Zed extension API 未提供 action/command palette 注册接口。
...
该路径使用 Zed 官方 task 机制，不经过 Agent slash command。
...
| --- | --- |
| 快捷键未占用 | 写入绑定 |
| 快捷键已绑定其他 action | 不覆盖，提示使用 `--force` 或指定新 key |
| task label 已存在且 command 相同 | 更新参数 |
| task label 已存在但 command 不同 | 不覆盖，提示人工处理 |
| JSON 格式非法 | 不写入，报告文件路径和解析错误 |

公开一键安装脚本和本地源码安装脚本默认调用 `install-zed --force`，用于在重复安装时覆盖并刷新所选快捷键；直接运行 `issue-jumper install-zed` 时仍保留不覆盖外部快捷键的安全默认值。

### 4.5 手动配置输出

如果安装器无法安全合并配置，`install-zed --print` 输出 task/keymap 片段，用于人工合并到 Zed 配置。
...
8. 未提取到 Issue ID 时，若 remote 是 GitHub/GitLab，则构建仓库首页 URL，否则失败
9. 已提取 Issue ID 时，解析目标平台并构建 Issue URL
...
- opener 失败时返回非 0 exit code，并输出 URL 供人工打开。
...
│                                                             │
│  keymap.json                                                │
│    alt alt -> task::Spawn("Issue Jumper: Open Current Issue") │
│                                                             │
│  tasks.json                                                 │
...
| 模块 | 文件位置 | 职责 |
| --- | --- | --- |
| `main.rs` | `src/main.rs` | 二进制入口，调用库 crate 的 CLI |
...
`custom_platforms[].host_patterns` 优先于内置平台识别，用于将私有 GitLab 覆盖为 GitLab work item、Jira 或企业内部工单 URL。
...
未识别到 Issue ID 时，GitHub remote 默认打开 `https://github.com/{owner}/{repo}`，GitLab remote 默认打开 `https://{host}/{project}`。
...
字段缺失时返回 `UrlBuildFailed`，禁止生成无效 URL。
...
| 平台未解析 | `Cannot determine issue platform. Configure fallback_platform or pass --platform.` |
...
| GitHub/GitLab remote + `main` | 构建远程仓库首页 URL |
...
| 非 GitHub/GitLab remote 无匹配分支 | 返回 exit code 1 |

浏览器打开测试默认使用 mock opener；真实 opener 纳入人工验收。

### 14.3 本地 mock 数据

本地 mock 数据放在 `local/mock-data/`，该目录通过 `.gitignore` 排除，不提交到 git。样例覆盖：
...
自动化测试不得依赖 `local/` 下的 ignored 文件；该目录仅用于本地人工验证和调试。
...
| 快捷键触发 | 在 Zed Workspace 或文件编辑器焦点中触发 `alt alt` keymap 绑定会执行 task |
| worktree 传参 | CLI 收到 `$ZED_WORKTREE_ROOT` 对应的项目路径 |
| 成功路径 | 浏览器打开 Issue URL，任务终端不显示 |
| 失败路径 | task 失败并保留可读错误输出 |
...
| 场景 | 影响 | 处理 |
| --- | --- | --- |
| 快捷键冲突 | 用户按键触发其他动作 | `install-zed` 默认不覆盖，提示更换 key 或使用 `--force` |
| CLI 不在 PATH | task 执行失败 | 安装器写入绝对路径，或 doctor 明确提示 |
| Zed task 终端显示 | 影响编辑器可见状态 | `reveal = "never"`、`hide = "on_success"` |
| 浏览器 opener 失败 | 页面未打开 | 输出 URL 并返回 exit code 4 |
| remote 缺失 | 平台无法推导 | 支持 Redmine/fallback_platform/custom template |
