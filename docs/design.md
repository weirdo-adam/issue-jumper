...
v0.1.1 的核心目标是：提供一个通用 CLI，从当前 Git 分支解析并打开对应 Issue 页面；同时提供 Zed 安装适配，在 Zed 中通过一个快捷键直接打开当前 Git 分支对应的 Issue 页面。成功路径不需要打开 Agent、复制 Issue ID 或点击中间链接。
...
Zed 集成由 `tasks.json` 和 `keymap.json` 完成。CLI 负责 Git 信息读取、Issue ID 匹配、URL 构建，并调用系统默认浏览器。当前公开 Zed extension API 的 slash command 入口出现在 Agent UI，不是截图中的 Command Palette，因此 v0.1.1 不采用 `/issue` 路径。后续其他编辑器或启动器集成应作为新的 `install-*` 适配层加入，不复制核心解析逻辑。
...
- v0.1.1 不发布 Zed marketplace extension。
- v0.1.1 不依赖未公开的 Zed Action API，也不宣称能注册独立 `issue: open` Command Palette action。
- 快捷键路径不要求用户打开 Agent、Assistant 或点击 Markdown 链接。
- v0.1.1 不调用 Issue 平台 API，不需要用户 token。
- v0.1.1 不采集客户数据，不上传分支名、仓库路径、Git remote、配置值或 Issue ID。
- v0.1.1 不依赖在线服务完成本地解析；安装脚本下载 Release 资产和浏览器访问远端 Issue 页面属于外部网络行为。
- v0.1.1 不自动创建 Issue，只打开已有 Issue URL。
...
截图里的 `Execute a command...` 是 Zed Command Palette。v0.1.1 不能注册独立的 `issue: open` action，因为当前公开 Zed extension API 没有 action/command palette 注册接口。
...
v0.1.1 支持高级定制 Issue 跳转，不支持泛化的任意目标跳转。
...
| 能力 | 说明 |
| --- | --- |
| 根据分支打开 PR 页面 | 不包含在 v0.1.1 |
| 根据分支打开 CI pipeline 页面 | 不包含在 v0.1.1 |
| 根据当前文件或选中文本打开文档、监控面板 | 不包含在 v0.1.1 |
| 多目标选择菜单 | 需要额外交互，不包含在 v0.1.1 |
